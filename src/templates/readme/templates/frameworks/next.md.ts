...
export const nextMd = (options: ProjectConfig)=>{
    const nameServer = capitalizeFirstLetter(options.selectedServer)
    const orm = mainOrm(options)
    return `
# Api \`${options.projectName}\` (${orm.part1} NEXT API)

A scalable API starter kit generated with CreateBackend CLI. Build for NEXT apps.
